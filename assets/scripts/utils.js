      ...
      if (thereIs === false) {
        ...
        favoriteArray.forEach((item) => {
          if (!favExist.includes(item)) {
            favExist.push(item);
            modal("Film aggiunto alla lista favoriti");
        ...
        modal("Film già presente nella lista favoriti");
      }
    })
  );
};
...
const popUp = (text) => {
  const popUp = cE("div");
  const popUpTitle = cE("h2");
  const deleteBtn = cE("button");
  const confirmBtn = cE("button");

  popUp.id = "popUp";
  popUpTitle.className = "modal-title";
  popUpTitle.textContent = text;

  popUp.appendChild(popUpTitle, deleteBtn, confirmBtn);
...
const removeFavorite = (element) => {
  const popUpBtnSec = cE("div");
  const deleteBtn = cE("button");
  const confirmBtn = cE("button");

  popUpBtnSec.id = "popUp-btns";
  deleteBtn.className = "delete-btn";
  confirmBtn.className = "confirm-btn";
  deleteBtn.textContent = "NO";
  confirmBtn.textContent = "SI";
  popUpBtnSec.append(deleteBtn, confirmBtn);
  document.body.appendChild(popUpBtnSec);

  popUp("Vuoi davvero rimuovere il film dalla lista?");

  confirmBtn.addEventListener("click", () => {
    document.getElementById("popUp").remove();
    document.getElementById("popUp-btns").remove();
    ...
    if (index !== -1) {
      ...
      modal("Film corretamente rimosso");
      setTimeout(() => {
        location.reload();
      }, 1000);
    }
